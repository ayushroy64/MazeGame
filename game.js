...
funcGameOver = function() {
    console.log("function called gameover");
    modal.style.display = "block";
    t = document.querySelector(".gamehead");
    t.textContent = "Game Over :(";
}

funcWin = function() {
    console.log("function called win");
    modal.style.display = "block";
    t = document.querySelector(".gamehead");
    t.textContent = "You Win!!";
}

document.getElementById("movecount").addEventListener("click",reloadFunc);
...
function startTimer(dur,disp){
    ...
    function timer() {
        if(play){
            ...
            disp.textContent = "Time Left: " + min + ":" + sec;

            if(diff<=0){
                disp.textContent = "Game Over :(";
                start = Date.now() + 1000;
                play = false;
                funcGameOver();
            }
...
window.onload = function() {
    t = 30;
    s = document.querySelector("#timer");
    startTimer(t,s);
}
...
function keyDown(event) {
    ...
    if(play) {
        ...
        if(m.checker("canvas")) play = false;
